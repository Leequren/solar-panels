...
export const NavBlock: React.FC<NavBlockProps> = ({ IsMobile }) => {
    return !IsMobile ? (
        <NavStyled>
            ...
            <NavItem name="Каталог" path="#Cart" counter={10} sCart imgUrl="/img/cartIcon.png" />
        </NavStyled>
    ) : (
        <NavStyled>
            <NavItem name="Главная" path="#Home" />
            <NavItem name="О сsервисе" path="#Additional" />
            <NavItem name="Продукция" path="#Products" />
            <NavItem name="О нас" path="#AboutUs" />
            <NavItem name="FAQ" path="#FAQ" />
            <NavItem name="Контакты" path="#ContactsItem" />
            <NavItem name="Каталог" path="#Cart" counter={10} isCart imgUrl="/img/cartIcon.png" />
        </NavStyled>
    )
}